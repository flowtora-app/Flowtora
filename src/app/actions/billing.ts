...
//
// Returns the latest subscription (or null if no customer / no sub /
// Stripe not configured). Errors are swallowed so page render doesn't
// blow up when Stripe is unreachable.
export async function reconcileTenantFromStripe(slug: string) {
  const ctx = await requirePermission(slug, "tenant:billing");
  if (!stripe || !ctx.tenant.stripeSubscriptionId) return;
  ...
  try {
    ...
    // Prefer explicit metadata pointer.
    ...
    // Silent — rendering must not fail if Stripe is unreachable or
    // the subscription has been cleaned up on their side.
  }
}
