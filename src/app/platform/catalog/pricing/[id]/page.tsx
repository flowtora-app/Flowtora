// Page 28 — Pricing Formula editor.
// 3 tabs: Definition (variables + constants + expression + tier table),
// Tester (live calculator), Versions.
...
import { loadPricingFormulaDetail } from "@/server/platform/pricing-formulas";
import {
  archivePricingFormula,
  duplicatePricingFormula,
  publishPricingFormula,
} from "@/app/actions/platform-pricing-formulas";
import { CATEGORY_LABEL, StatusPill } from "../_components/shared";
import { DefinitionTab } from "./_components/DefinitionTab";
import { TesterTab } from "./_components/TesterTab";
import { VersionsTab } from "./_components/VersionsTab";

export const dynamic = "force-dynamic";

type SP = Record<string, string | string[] | undefined>;
type TabKey = "definition" | "tester" | "versions";
const TAB_KEYS: TabKey[] = ["definition", "tester", "versions"];
...
export default async function PricingFormulaEditorPage({
  params, searchParams,
}: {
  ...
  const detail = await loadPricingFormulaDetail(id);
  if (!detail) notFound();

  const tabRaw = typeof sp.tab === "string" ? sp.tab : "definition";
  const tab: TabKey = (TAB_KEYS as readonly string[]).includes(tabRaw) ? (tabRaw as TabKey) : "definition";
    ...
    <div className="min-w-0 space-y-5">
      <div>
        <Breadcrumb items={[
          ...
          { label: "Pricing Formulas", href: "/platform/catalog/pricing" },
          { label: detail.name },
        ]} />
        <div className="mt-3">
          <PageHeader
            ...
            description={
              <span className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-[12px]" style={{ color: "var(--text-muted)" }}>
                  ...
                  {detail.variables.length} input{detail.variables.length === 1 ? "" : "s"} ·{" "}
      ...
      <TabBar formulaId={detail.id} active={tab} />

      {tab === "definition" && <DefinitionTab detail={detail} canManage={canManage} />}
      {tab === "tester"     && <TesterTab detail={detail} />}
      {tab === "versions"   && (
...
function TabBar({ formulaId, active }: { formulaId: string; active: TabKey }) {
  return (
        ...
        {TAB_KEYS.map((key) => {
          ...
          return (
            <Link
              key={key}
              href={key === "definition"
                ? `/platform/catalog/pricing/${formulaId}`
                : `/platform/catalog/pricing/${formulaId}?tab=${key}`}
...
function ActionRow({ id, status }: { id: string; status: "DRAFT" | "PUBLISHED" | "ARCHIVED" }) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      {status !== "PUBLISHED" && (
        <form action={publishPricingFormula.bind(null, id)}>
          <button type="submit"
      ...
      <form action={duplicatePricingFormula.bind(null, id)}>
        <button type="submit"
      ...
      {status !== "ARCHIVED" && (
        <form action={archivePricingFormula.bind(null, id)}>
          <button type="submit"
                  className="ts-focus rounded-md border px-3 py-1.5 text-[12px] font-medium"
