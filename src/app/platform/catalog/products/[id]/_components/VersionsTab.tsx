  ...
  return (
    <div className="space-y-5">
      <section className="rounded-lg border" style={{ background: "var(--surface-1)", borderColor: "var(--border-subtle)" }}>
        <div className="border-b px-4 py-3" style={{ borderColor: "var(--border-subtle)" }}>
          ...
          <p className="text-[12px]" style={{ color: "var(--text-muted)" }}>
            Each Publish appends a snapshot. Showing newest first, capped at 25.
          </p>
        </div>
        {versions.length === 0 ? (
      ...
      <DeferredNote>
        ...
        under <span className="font-mono">MasterProductVersion.snapshot</span> — the diff UI ships
        when the JSON-diff component lands (same dependency the Plans changelog is waiting on).
      </DeferredNote>
    </div>
  );
}
