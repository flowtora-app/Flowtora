// Page 44 — shared bits for the Lead Inbox.

import * as React from "react";
import { statusLabel, statusTone } from "@/server/platform/leads-inbox";
...
export function StatusPill({ status }: { status: MarketingLeadStatus }) {
...
export function ScoreBadge({ score }: { score: number }) {
  const tone = score >= 70 ? "var(--success-fg)" :
               score >= 40 ? "var(--accent-primary)" :
               score >= 20 ? "var(--warning-fg)" : "var(--text-muted)";
  return (
    <span
      className="inline-flex items-center justify-center rounded-md px-1.5 py-0.5 text-[11px] font-semibold tabular-nums"
      style={{
