...
// 6-tile KPI band at the top of /platform/health.
//
//   Status        Active users  Errors (1h)  Failed jobs  Login fails  Open incidents
//   ● Healthy     142            3            0            18           0
...
export interface HealthKpi {
  ...
  deltaPct?: number;
  /** Inverts green/red — increases in errors are bad. */
  deltaInvert?: boolean;
  tone?: "default" | "accent" | "success" | "warning" | "danger";
  /** Show a status dot beside the value (used for the Status tile). */
  dot?: boolean;
}

const TONE: Record<NonNullable<HealthKpi["tone"]>, { bg: string; border: string; label: string }> = {
...
export function HealthKPIBand({ kpis }: { kpis: HealthKpi[] }) {
  return (
...
function Tile({ kpi }: { kpi: HealthKpi }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex items-baseline gap-2">
        ...
        <span
          className="truncate text-xl font-semibold tabular-nums tracking-tight"
