...
// KPI band for the marketing-leads inbox.
//
// Five tiles tinted by what the metric measures:
//   Total · New (today) · Unassigned · Demo requests · Conversion rate
//
// Each tile can show a trailing-period delta pill. `deltaInvert: true`
// flips the green/red mapping for metrics where "up" is bad — e.g. a
// rising Unassigned count is a service-quality regression.
...
export interface LeadsKpi {
  ...
  deltaPct?: number;
  /** Inverts the green/red mapping — increases in unassigned are bad. */
  deltaInvert?: boolean;
  tone?: "default" | "accent" | "success" | "warning" | "danger";
}

const TONE: Record<NonNullable<LeadsKpi["tone"]>, { bg: string; border: string; label: string }> = {
...
export function LeadsKPIBand({ kpis }: { kpis: LeadsKpi[] }) {
  return (
...
function KpiTile({ kpi }: { kpi: LeadsKpi }) {
