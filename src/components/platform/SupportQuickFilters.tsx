...
// Pill-chip filter rows for the support queue. Two rows:
//   1. Status — Active · Open · In progress · Waiting · Resolved · Closed
//   2. Priority — Any · Urgent · High · Normal · Low
//
// Each chip toggles its URL param (?scope= and ?priority=) and clears
// any pagination so filtering doesn't strand the user on a stale page.
// Counts come from server-side groupBy and dim the chip when zero.

export type SupportChipGroup = "scope" | "priority" | "category";

export interface SupportChip {
  /** Raw value for the URL param. Empty string clears the param. */
  value: string;
  label: string;
  count?: number;
...
export function SupportQuickFilters({
  ...
  paramKey: SupportChipGroup;
  chips: SupportChip[];
  /** What "no value" should match against — usually "ALL" or "". */
  defaultValue?: string;
}) {
...
const TONE: Record<NonNullable<SupportChip["tone"]>, { activeBg: string; activeFg: string; idleFg: string }> = {
