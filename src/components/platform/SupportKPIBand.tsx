...
// 5-tile KPI band at the top of the support queue.
//
//   Open                Unassigned          Urgent                Today              Avg first reply
//   42                  7                   3 (1 SLA red)         12 (+30%)          1h 24m (-12%)
//
// Same shape as the other admin KPI bands. `deltaInvert` flips the
// red/green mapping for tiles where increases are bad — unassigned,
// SLA breaches, etc.
...
const TONE: Record<NonNullable<SupportKpi["tone"]>, { bg: string; border: string; label: string }> = {
...
export function SupportKPIBand({ kpis }: { kpis: SupportKpi[] }) {
  return (
...
function Tile({ kpi }: { kpi: SupportKpi }) {
