...
// KPI band for the tenants admin page.
//
// Five tiles across:
//   Total tenants · Active · Trial · Past due · New (30d)
//
// Each tile shows a big number, a trend pill (period-over-period
// delta where computable), and a hint line. Color tones come from the
// status they represent — Active green, Trial accent, Past due amber.
//
// Computation lives on the page; this component is a dumb renderer.

export interface TenantKpi {
  label: string;
  value: string;
  hint?: string;
  /** Delta as a fraction; 0.12 = +12%. Pass undefined to omit the pill. */
  deltaPct?: number;
  /** Inverts the green/red mapping when an increase is bad (past-due, etc.). */
  deltaInvert?: boolean;
  tone?: "default" | "accent" | "success" | "warning" | "danger";
}

const TONE: Record<NonNullable<TenantKpi["tone"]>, { bg: string; border: string; label: string }> = {
...
export function TenantsKPIBand({ kpis }: { kpis: TenantKpi[] }) {
  return (
      ...
      <div className="mt-2 flex items-baseline gap-2">
        ...
        {hasDelta && (
          <span
            ...
            title={`${(kpi.deltaPct! * 100).toFixed(1)}% vs prior 30d`}
          >
